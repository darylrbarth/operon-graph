...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build operon graph from PROST clusters + Prodigal-style protein FASTA headers (adjacent-gene edges)."
    )
    parser.add_argument("--prost_tsv", required=True, help="Path to PROST TSV (columns: head, member, score, e_value).")
    parser.add_argument("--proteins_faa", required=True, help="Protein FASTA produced by Prodigal/Prokka (Prodigal-style headers).")
    parser.add_argument("--operonic_distance", type=int, default=40, help="Max bp between adjacent genes to be considered in same operon (default: 40).")
    parser.add_argument("--out_prefix", default="operon_network", help="Prefix for output files (default: operon_network).")
    args = parser.parse_args()

    builder = GraphBuilderProst(
        prost_tsv=args.prost_tsv,
        header_file=args.proteins_faa,
    ...
    out_gml = f"{out_prefix}.gml"
    out_nodes = f"{out_prefix}_nodes.tsv"
    out_edges = f"{out_prefix}_edges.tsv"

    builder.export_gml(out_gml)
    builder.export_tsv(out_nodes, out_edges)
    print(f"Wrote: {out_gml}, {out_nodes}, {out_edges}")
